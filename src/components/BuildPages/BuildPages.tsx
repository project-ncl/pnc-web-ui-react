...
const BuildSshCredentialsButton = ({ serviceContainerBuild }: IBuildSshCredentialsButtonProps) => {
  ...
  const areSshCredentialsAvailable = useMemo(() => process.env.REACT_APP_SSH_CREDENTIALS_AVAILABLE === 'true', []);

  useEffect(() => {
    if (
      areSshCredentialsAvailable &&
  ...
  return (
    <SshCredentialsButton
      ...
      areSshCredentialsAvailable={areSshCredentialsAvailable}
    />
  );
};
