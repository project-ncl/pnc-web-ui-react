...
class EttClient {
  ...
  private createHttpClient = (): AxiosInstance | null => {
    const url = webConfigService.getEttUrl();

    if (!url) {
      uiLogger.error('ETT URL not available in the config.');
      return null;
    }
  ...
  public getHttpClient = (): Result<AxiosInstance, HttpClientCreationFailedError> => {
    if (!this.httpClient) {
      return {
        success: false,
        error: new HttpClientCreationFailedError({ message: 'ETT HTTP client could not be instantiated.' }),
      };
    }
