...
/**
 * Dingrogu HTTP client.
 */
class DingroguClient {
  ...
  private createHttpClient = (): AxiosInstance | null => {
    const url = webConfigService.getDingroguUrl();

    if (!url) {
      uiLogger.error('Dingrogu URL not available in the config.');
      return null;
    }
  ...
  public getHttpClient = (): Result<AxiosInstance, HttpClientCreationFailedError> => {
    if (!this.httpClient) {
      return {
        success: false,
        error: new HttpClientCreationFailedError({ message: 'Dingrogu HTTP client could not be instantiated.' }),
      };
    }
...
export const dingroguClient = new DingroguClient();
